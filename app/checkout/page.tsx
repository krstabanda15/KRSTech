...
export default function Checkout() {
  ...
  const subtotal = cart.reduce((sum, item) => sum + (item.customization?.unitPrice || products.find(product => product.id === item.id)?.price || 0) * item.qty, 0);
...
function OrderSummary({ cart, subtotal, delivery }: { cart: import("@/types").CartLine[]; subtotal: number; delivery: string }) { const shipping = delivery === "Express" ? 299 : 0; return <aside className="panel h-fit p-6"><h2 className="font-bold">Order summary</h2><div className="my-5 space-y-3">{cart.map(item => { const product = products.find(product => product.id === item.id) || (item.customization ? { name:item.customization.productName, price:item.customization.unitPrice } : undefined); return product ? <div className="border-b border-[#202633] pb-3 text-sm" key={item.id}><div className="flex justify-between"><span className="muted">{item.qty}× {product.name}</span><b>{peso(product.price * item.qty)}</b></div>{item.customization&&<p className="mt-1 text-xs text-[#77e5ad]">{item.customization.setup==="kada"?"KADA programmed - FREE":"Self-programmed"} - {item.customization.color}</p>}</div> : null; })}</div><div className="border-t border-[#303745] pt-4"><div className="flex justify-between"><span className="muted">Shipping</span><span>{shipping ? peso(shipping) : "Free"}</span></div><div className="mt-4 flex justify-between text-xl"><b>Total</b><b>{peso(subtotal + shipping)}</b></div></div></aside>; }
