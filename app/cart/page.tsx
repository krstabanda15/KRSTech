...
export default function Cart() {
  ...
  const lines = cart.map(line => ({ ...line, product: products.find(product => product.id === line.id) || (line.customization ? { id:line.id,name:line.customization.productName,brand:"KADA CUSTOM NFC",category:"NFC",price:line.customization.unitPrice,rating:5,reviewCount:0,stock:99,description:"Personalized NFC product",image:"/images/krst.png",connection:"NFC",specifications:{},highlights:[],bestFor:"Custom connections",buyerTip:"" } : undefined) })).filter(line => line.product);
  ...
  return <div className="container py-12"><nav className="muted mb-5 text-sm"><Link href="/">Home</Link> / Cart</nav><p className="eyebrow">Your bag</p><h1 className="mt-2 text-4xl font-black">Shopping cart</h1>
    ...
    <><div className="mt-8 grid gap-7 lg:grid-cols-[1fr_380px]"><div className="space-y-4">
      <div className="panel p-5"><div className="flex justify-between gap-4 text-sm"><div><b>{remaining ? `Add ${peso(remaining)} more to unlock FREE shipping` : "You have unlocked FREE shipping!"}</b><p className="muted mt-1 text-xs">{peso(Math.min(subtotal,FREE_SHIPPING_THRESHOLD))} / {peso(FREE_SHIPPING_THRESHOLD)} - Demo promotion</p></div><Truck size={19} className="text-[#77e5ad]" /></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#252c38]"><div className="h-full rounded-full bg-[#77e5ad] transition-all" style={{ width: `${Math.min(100, subtotal / FREE_SHIPPING_THRESHOLD * 100)}%` }} /></div></div>
