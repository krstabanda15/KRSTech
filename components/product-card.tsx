...
import { discountPercent, studentPrice } from "@/data/pricing";
...
export function ProductCard({ p, list = false }: { p: Product; list?: boolean }) {
  ...
  return <article className={`group panel overflow-hidden ${list ? "sm:flex" : ""}`}>
    <div className={`relative overflow-hidden bg-[#151a22] ${list ? "sm:w-64" : "aspect-[4/3]"}`}>
      ...
      {p.oldPrice && <span className="badge absolute left-3 top-3">SAVE {discountPercent(p.price,p.oldPrice)}%</span>}
    ...
    <div className="flex flex-1 flex-col p-5">
      ...
      <div className="mt-auto flex items-end justify-between pt-5"><div><div><b className="text-xl">{peso(p.price)}</b>{p.oldPrice && <del className="muted ml-2 text-sm">{peso(p.oldPrice)}</del>}</div>{p.studentEligible&&<span className="mt-1 block text-xs text-[#77e5ad]">Student price {peso(studentPrice(p.price))}*</span>}<span className={"mt-1 block text-xs "+(p.stock?"text-gray-400":"text-red-400")}>{p.stock?p.stock+" in stock - Shipping shown in cart":"Out of stock"}</span></div><div className="flex gap-2"><button aria-label={compared ? "Remove from comparison" : "Add to comparison"} title={compared ? "Added to compare" : "Compare"} onClick={() => toggleCompare(p.id)} className={`relative rounded-lg border p-2 ${compared ? "border-[#77e5ad] bg-[#77e5ad] text-black" : "border-[#303745] hover:border-[#77e5ad]"}`}>{compared ? <Check size={17} /> : <GitCompareArrows size={17} />}</button><button title="Quick add" onClick={() => addCart(p.id)} className="rounded-lg bg-[#77e5ad] p-2 text-black hover:bg-white"><Plus size={17} /></button></div></div>
    </div>
  </article>;
}
